...
module SSubReverse {
   ...
   * Write enough annotatations (~1) to help Dafny prove subreverseLemma (which will also need ~1 annotation)
...
module SRotate {
   ...
   * Write enough annotatations (~1) to help Dafny prove rotateLemma (which will also need ~1 annotation)
...
module Reverse {
  ...
  import SSub = SSubReverse
  import SRot = SRotate
  import SwapRev = SwapSubReverse
  /**
   * reverse implements in-place, linear time reversal of the input array v.
   * Complete the specification (~1 line) of reverse to enable Dafny 
 ...
 module Rotate {
  ...
  {
      var n := v.Length;
      SwapRev.subreverse(v, 0, n - k);
      SwapRev.subreverse(v, n - k, n);
      Rev.reverse(v);
  }
